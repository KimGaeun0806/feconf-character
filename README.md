...
### 3) 💯 Web Vitals 실시간 피드백

Vite 플러그인을 쓰면 dev 서버로 띄운 페이지의 **실제 성능 지표**를 달팽이가 지켜봅니다.
...
- 끄려면 `mascot({ vitals: false })`.
- 브라우저 콘솔에서 `window.__mascotVitals.metrics` 로 지금까지 잡힌 값을 볼 수 있어요.

Vite 를 안 쓴다면 값을 직접 보내도 됩니다.
